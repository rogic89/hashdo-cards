...
card.onReady = function () {
  // if booking is NOT allowed then just skip everything
  if (locals.config.isBookingAllowed) {

    var $card = $('#' + locals.card.id),
        $container = $card.find('.container'),
        $shiftMonths = $container.find('.shifting-select.months'),
        $shiftDays = $container.find('.shifting-select.days'),
        $shiftHours = $container.find('.shifting-select.hours'),
        // selected values
        now = new Date(),
    ...
    flip = function(side) {
      $container.addClass("scale");  // add/remove class in delayed fashion

      setTimeout(function() {
        side === 'back' ? $container.addClass("flip") : $container.removeClass("flip");
      }, 500);
